...
$pdf->Cell(80);
$pdf->Cell(30,0,utf8_decode(' V E N T A S   D E L  D Í A '),.2,0,'C');// Título	
$pdf->Ln(10);
...
$pdf->SetXY(10, 65);
		...
		$pdf->Cell(40,7, utf8_decode('Cantidad'),1, 0 , 'L', true);
...
$sql=$base->prepare('select * from salidas where fecha="'.$fecha2.'"');
$sql->execute();
	...
	$tabla=$base->prepare('SELECT Tabla from codigos where Codigo="'.$row['codigo'].'"');
 	...
 	while ($row3=$tabla->fetch(PDO::FETCH_ASSOC)) {
 		...
 		while ($row2=$descripcion->fetch(PDO::FETCH_ASSOC)) {
 			...
 			$pdf->Cell(40,7, utf8_decode($row['cantidad']),1, 0 , 'L', $bandera );
 			$pdf->Cell(40,7, utf8_decode($row['total']),1, 0 , 'L', $bandera );
